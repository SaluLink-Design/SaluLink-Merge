...
- **Native Fetch API** - frontend API requests
...
- Update `app/api/analyze/route.ts` or set `PYTHON_BACKEND_URL` if the backend is hosted on a different port or host
...
- Python backend currently allows all origins via CORS for development
