...
import { enrollmentToBenefitState, reconcileMedicationsForBenefitState } from '@/lib/benefitState';
  ...
  persist(
    (set, get) => ({
      ...
      saveCase: (patientName, patientId, claimType = 'diagnostic') => {
        ...
        const priorCase = state.cases.find((c) => c.id === state.currentCaseId);

        const newCase: PatientCase = {
          id: Date.now().toString(),
          profileId: priorCase?.profileId,
          patientName,
          patientId,
          claimType,
          createdAt: new Date(),
          ...
          status,
          cibRecords: (() => {
            const existing = state.cases.find((c) => c.id === state.currentCaseId)?.cibRecords ?? [];
      ...
      loadCase: (caseId) => {
        ...
        if (selectedCase) {
          ...
          set({
            ...
            activeBenefitState: (() => {
              if (selectedCase.cibEnrollmentStatus) {
                return enrollmentToBenefitState(selectedCase.cibEnrollmentStatus);
              }
              const rec = selectedCase.cibRecords?.find(
                (r) => r.conditionName === selectedCase.condition
              );
              return rec?.benefitState ?? null;
            })(),
            diagnosisDate:
