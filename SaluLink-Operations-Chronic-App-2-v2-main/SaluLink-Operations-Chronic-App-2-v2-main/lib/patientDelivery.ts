...
const defaultSelection = (data: PatientExportData) => ({
...
export async function deliverClaimToPatient(
  ...
  const zipBlob = await generatePatientExportZip(data, defaultSelection(data));
  ...
  try {
    ...
    deliverViaMailto(data, patientEmail, practiceName, filename, zipBlob);
