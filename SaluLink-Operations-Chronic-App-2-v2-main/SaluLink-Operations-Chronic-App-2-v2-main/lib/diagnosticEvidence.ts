...
export const deriveEvidenceFromDiagnostics = (treatments: TreatmentItem[]) => {
  ...
  const hasLabResults =
    labTests.length === 0 ? false : labDocumented.length > 0;
  const hasImaging =
...
export const canProceedFromEvidenceReview = (
  ...
  if (!derived.hasLabResults) {
    return { ok: false, reason: 'Attach pathology or lab findings for at least one lab test.' };
  }
